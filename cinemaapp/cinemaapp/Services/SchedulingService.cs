...
namespace test.Services
{
    ...
    AND NOT (
        ts.endTime <= :newStartTime
        OR ts.startTime >= :newEndTime
    )";
        ...
        public (bool Success, string Message) DeleteSection(int sectionId)
        {
            using (OracleConnection connection = new OracleConnection(_connectionString))
            {
                ...
                try
                {
                    ...
                    string getTicketIdsSql = "SELECT ticketID FROM ticket WHERE sectionID = :sectionID";
                    using (OracleCommand getTicketIdsCmd = new OracleCommand(getTicketIdsSql, connection))
                    {
                        getTicketIdsCmd.Transaction = transaction;
                        getTicketIdsCmd.Parameters.Add(new OracleParameter("sectionID", sectionId));
                        using (var reader = getTicketIdsCmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                    ...
                    {
                        string deleteOrdersSql = "DELETE FROM orderfortickets WHERE ticketID IN (" +
                                              string.Join(",", ticketIds.Select(id => $"'{id}'")) + ")";
                        using (OracleCommand deleteOrdersCmd = new OracleCommand(deleteOrdersSql, connection))
                        {
                            deleteOrdersCmd.Transaction = transaction;
                ...
                {
                    transaction?.Rollback();
                    return (false, $"数据库错误: {ex.Message} (错误代码: {ex.Number})");
                }
                catch (Exception ex)
                {
                    transaction?.Rollback();
